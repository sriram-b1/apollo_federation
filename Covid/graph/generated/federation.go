...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) ([]fedruntime.Entity, error) {
	...
	for _, rep := range representations {
		...
		case "Covid":
			id0, err := ec.unmarshalOString2ᚖstring(ctx, rep["date"])
			if err != nil {
				return nil, errors.New(fmt.Sprintf("Field %s undefined in schema.", "date"))
			}

			entity, err := ec.resolvers.Entity().FindCovidByDate(ctx,
				id0)
			if err != nil {
				return nil, err
			}
